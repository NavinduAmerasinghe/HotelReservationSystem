...
//getUsers
router.get("/", (req, res) => {
  // using the schema's find function to find all the docs in user collection
  User.find((err, docs) => {
...
router.post("/", (req, res) => {
  ...
  const token = new jsonwebtoken( // creating json web Token with required details for user manegement
    {
      id: user.id,
      email: user.email,
      type: user.type,
...
router.put("/:id", (req, res) => {
  User.findOneAndUpdate(
    //using find one and update to find a relevant doc and update it with new data
    {
      id: req.params.id,
    },
    {
